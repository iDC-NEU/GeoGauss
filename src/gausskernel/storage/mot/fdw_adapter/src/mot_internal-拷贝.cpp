...
bool MOTAdaptor::InsertRowToMergeRequestTxn(MOT::TxnManager* txMan, const uint64_t& index_pack, const uint64_t& index_unique){
    ...
    // std::unique_ptr<zmq::message_t> msg = std::make_unique<zmq::message_t>(
    //     static_cast<void*>(const_cast<char*>(serialized_txn_str_ptr->data())), serialized_txn_str_ptr->size(), 
    //     string_free, static_cast<void*>(serialized_txn_str_ptr));
...
void GenerateSendTasks(uint64_t kPackageNum){
    ...
    for(int i = 0; i <= (int)kPackageNum; i++){
        (*ptr1)[i] = std::make_shared<BlockingConcurrentQueue<std::unique_ptr<zmq::message_t>>>(kMergeThreadNum * 
