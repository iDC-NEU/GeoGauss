...
std::vector<uint64_t> epoch_physical_thread_ids;
...
void EpochPhysicalTimerManagerMain(void)
{
     ...
     */
    t_thrd.utils_cxt.CurrentResourceOwner = ResourceOwnerCreate(NULL, "epoch physical timer manager", MEMORY_CONTEXT_STORAGE);
     ...
     */
    epochphysicaltimermanager_context = AllocSetContextCreate(t_thrd.top_mem_cxt,
        "epoch physical timer manager",
        ALLOCSET_DEFAULT_MINSIZE,
        ALLOCSET_DEFAULT_INITSIZE,
        ALLOCSET_DEFAULT_MAXSIZE);
    (void)MemoryContextSwitchTo(epochphysicaltimermanager_context);
    ...
    if (sigsetjmp(local_sigjmp_buf, 1) != 0) {
         ...
         * about in epochphysicaltimermanager, but we do have LWLocks, and perhaps buffers?
         */
        LWLockReleaseAll();
        ...
        MemoryContextResetAndDeleteChildren(epochphysicaltimermanager_context);
     ...
     * sleeping.
     */
    // g_instance.proc_base->epochphysicaltimermanagerLatch = &t_thrd.proc->procLatch;

    pgstat_report_appname("epoch physical timer manager");
    ...
    for(auto &i : epoch_physical_thread_ids ){
        if(i == thread_id){
            break;
        }
        id++;
    }
    if(id < (uint64_t)epoch_physical_thread_ids.size())
        ereport(LOG, (errmsg("epoch physical pid %llu %llu %lu", id, epoch_physical_thread_ids[id], thread_id)));
    for (;;) {
         ...
         */
        if (t_thrd.epochphysicaltimermanager_cxt.got_SIGHUP) {
            t_thrd.epochphysicaltimermanager_cxt.got_SIGHUP = false;
            ProcessConfigFile(PGC_SIGHUP);
        }

        if (t_thrd.epochphysicaltimermanager_cxt.shutdown_requested) {
            /* Normal exit from the epochphysicaltimermanager is here */
            proc_exit(0); /* done */
        }

        FDWEpochPhysicalTimerManagerThreadMain(id, kServerIp, kServerNum, kPackageNum, kNotifyThreadNum, kPackThreadNum, local_ip_index, (uint64_t)kSleepTime);
        ...
        //         while (pCriticalEntry->status == WAL_NOT_COPIED && !t_thrd.epochphysicaltimermanager_cxt.shutdown_requested) {
...
static void _SigHupHandler(SIGNAL_ARGS)
{
    ...
    t_thrd.epochphysicaltimermanager_cxt.got_SIGHUP = true;
...
static void _ShutdownHandler(SIGNAL_ARGS)
{
    ...
    t_thrd.epochphysicaltimermanager_cxt.shutdown_requested = true;
