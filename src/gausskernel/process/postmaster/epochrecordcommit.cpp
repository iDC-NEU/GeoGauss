...
std::vector<uint64_t> epoch_record_commit_thread_ids;
...
void EpochRecordCommitMain(void)
{
    sigjmp_buf local_sigjmp_buf;
    MemoryContext epochrecordcommit_context;
     ...
     */
    t_thrd.utils_cxt.CurrentResourceOwner = ResourceOwnerCreate(NULL, "epoch record commit", MEMORY_CONTEXT_STORAGE);
     ...
     */
    epochrecordcommit_context = AllocSetContextCreate(t_thrd.top_mem_cxt,
        "epoch record commit",
        ALLOCSET_DEFAULT_MINSIZE,
        ALLOCSET_DEFAULT_INITSIZE,
        ALLOCSET_DEFAULT_MAXSIZE);
    (void)MemoryContextSwitchTo(epochrecordcommit_context);
    ...
    if (sigsetjmp(local_sigjmp_buf, 1) != 0) {
         ...
         * about in epochcommit, but we do have LWLocks, and perhaps buffers?
         */
        LWLockReleaseAll();
        ...
        MemoryContextResetAndDeleteChildren(epochrecordcommit_context);
    ...
    pgstat_report_appname("epoch record commit");
    pgstat_report_activity(STATE_IDLE, NULL);

    /*
     * Loop forever
     */
    // ereport(LOG, (errmsg("epoch  commit pid %d", t_thrd.epochcommit_cxt.epochcommit_id)));
    uint64_t id = 0;
    std::stringstream thread_id_stream;
    thread_id_stream << std::this_thread::get_id();
    uint64_t thread_id = std::stoull(thread_id_stream.str());
    for(auto &i : epoch_record_commit_thread_ids ){
        if(i == thread_id){
            break;
        }
        id++;
    ...
    for (;;) {
         ...
         */
        if (t_thrd.epochcommit_cxt.got_SIGHUP) {
            t_thrd.epochcommit_cxt.got_SIGHUP = false;
            ProcessConfigFile(PGC_SIGHUP);
        }

        if (t_thrd.epochcommit_cxt.shutdown_requested) {
            /* Normal exit from the epochcommit is here */
            proc_exit(0); /* done */
        }
        FDWEpochRecordCommitThreadMain(id);
...
static void _SigHupHandler(SIGNAL_ARGS)
{
    ...
    t_thrd.epochrecordcommit_cxt.got_SIGHUP = true;
...
static void _ShutdownHandler(SIGNAL_ARGS)
{
    ...
    t_thrd.epochrecordcommit_cxt.shutdown_requested = true;
