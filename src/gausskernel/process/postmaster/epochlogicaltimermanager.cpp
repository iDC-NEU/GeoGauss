...
std::vector<uint64_t> epoch_logical_thread_ids;
 ...
 */
void EpochLogicalTimerManagerMain(void)
{
    sigjmp_buf local_sigjmp_buf;
    MemoryContext epochlogicaltimermanager_context;
     ...
     */
    t_thrd.utils_cxt.CurrentResourceOwner = ResourceOwnerCreate(NULL, "epoch logical timer manager", MEMORY_CONTEXT_STORAGE);
     ...
     */
    epochlogicaltimermanager_context = AllocSetContextCreate(t_thrd.top_mem_cxt,
        "epoch logical timer manager",
        ALLOCSET_DEFAULT_MINSIZE,
        ALLOCSET_DEFAULT_INITSIZE,
        ALLOCSET_DEFAULT_MAXSIZE);
    (void)MemoryContextSwitchTo(epochlogicaltimermanager_context);
    ...
    if (sigsetjmp(local_sigjmp_buf, 1) != 0) {
         ...
         * about in epochlogicaltimermanager, but we do have LWLocks, and perhaps buffers?
         */
        LWLockReleaseAll();
        ...
        MemoryContextResetAndDeleteChildren(epochlogicaltimermanager_context);
     ...
     * sleeping.
     */
    // g_instance.proc_base->epochlogicaltimermanagerLatch = &t_thrd.proc->procLatch;

    pgstat_report_appname("epoch logical timer manager");
    ...
    for(auto &i : epoch_logical_thread_ids ){
        if(i == thread_id){
            break;
        }
        id++;
    ...
    for (;;) {
         ...
         */
        if (t_thrd.epochlogicaltimermanager_cxt.got_SIGHUP) {
            t_thrd.epochlogicaltimermanager_cxt.got_SIGHUP = false;
            ProcessConfigFile(PGC_SIGHUP);
        }

        if (t_thrd.epochlogicaltimermanager_cxt.shutdown_requested) {
            /* Normal exit from the epochlogicaltimermanager is here */
            proc_exit(0); /* done */
        }
        FDWEpochLogicalTimerManagerThreadMain(id);
...
static void _SigHupHandler(SIGNAL_ARGS)
{
    ...
    t_thrd.epochlogicaltimermanager_cxt.got_SIGHUP = true;
...
static void _ShutdownHandler(SIGNAL_ARGS)
{
    ...
    t_thrd.epochlogicaltimermanager_cxt.shutdown_requested = true;
