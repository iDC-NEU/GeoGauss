...
std::vector<uint64_t> epoch_unseri_thread_ids;
...
void EpochUnseriMain(void)
{
    sigjmp_buf local_sigjmp_buf;
    MemoryContext epochunseri_context;
    ...
    (void)MemoryContextSwitchTo(epochunseri_context);
    ...
    if (sigsetjmp(local_sigjmp_buf, 1) != 0) {
         ...
         * about in epochunseri, but we do have LWLocks, and perhaps buffers?
         */
        LWLockReleaseAll();
        ...
        MemoryContextResetAndDeleteChildren(epochunseri_context);
    ...
    for(auto &i : epoch_unseri_thread_ids ){
        if(i == thread_id){
            break;
        }
        id++;
    ...
    for (;;) {
         ...
         */
        if (t_thrd.epochunseri_cxt.got_SIGHUP) {
            t_thrd.epochunseri_cxt.got_SIGHUP = false;
            ProcessConfigFile(PGC_SIGHUP);
        }

        if (t_thrd.epochunseri_cxt.shutdown_requested) {
            /* Normal exit from the epochunseri is here */
            proc_exit(0); /* done */
        }
        FDWEpochUnseriThreadMain(id);
...
static void _SigHupHandler(SIGNAL_ARGS)
{
    ...
    t_thrd.epochunseri_cxt.got_SIGHUP = true;
...
static void _ShutdownHandler(SIGNAL_ARGS)
{
    ...
    t_thrd.epochunseri_cxt.shutdown_requested = true;
