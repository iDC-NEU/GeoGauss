...
std::vector<uint64_t> epoch_unpack_thread_ids;
...
void EpochUnpackMain(void)
{
    sigjmp_buf local_sigjmp_buf;
    MemoryContext epochunpack_context;
    ...
    // ereport(LOG, (errmsg("epochunpack started")));
    ...
    (void)MemoryContextSwitchTo(epochunpack_context);
    ...
    if (sigsetjmp(local_sigjmp_buf, 1) != 0) {
         ...
         * about in epochunpack, but we do have LWLocks, and perhaps buffers?
         */
        LWLockReleaseAll();
        ...
        MemoryContextResetAndDeleteChildren(epochunpack_context);
    ...
    for(auto &i : epoch_unpack_thread_ids ){
        if(i == thread_id){
            break;
        }
        id++;
    }
    if(id < (uint64_t)epoch_unpack_thread_ids.size())
        ereport(LOG, (errmsg("epoch unpack pid %llu %llu %lu", id, epoch_unpack_thread_ids[id], thread_id)));
    for (;;) {
         ...
         */
        if (t_thrd.epochunpack_cxt.got_SIGHUP) {
            t_thrd.epochunpack_cxt.got_SIGHUP = false;
            ProcessConfigFile(PGC_SIGHUP);
        }

        if (t_thrd.epochunpack_cxt.shutdown_requested) {
        ...
        //         while (pCriticalEntry->status == WAL_NOT_COPIED && !t_thrd.epochunpack_cxt.shutdown_requested) {
...
static void _SigHupHandler(SIGNAL_ARGS)
{
    ...
    t_thrd.epochunpack_cxt.got_SIGHUP = true;
...
static void _ShutdownHandler(SIGNAL_ARGS)
{
    ...
    t_thrd.epochunpack_cxt.shutdown_requested = true;
