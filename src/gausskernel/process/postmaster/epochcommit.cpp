...
std::vector<uint64_t> epoch_commit_thread_ids;
...
void EpochCommitMain(void)
{
    sigjmp_buf local_sigjmp_buf;
    MemoryContext epochcommit_context;
    ...
    (void)MemoryContextSwitchTo(epochcommit_context);
    ...
    if (sigsetjmp(local_sigjmp_buf, 1) != 0) {
        ...
        MemoryContextResetAndDeleteChildren(epochcommit_context);
    ...
    for(auto &i : epoch_commit_thread_ids ){
        if(i == thread_id){
            break;
        }
        id++;
    }
    // if(id < (uint64_t)epoch_commit_thread_ids.size())
    //     ereport(LOG, (errmsg("epoch commit pid %llu %llu %lu", id, epoch_commit_thread_ids[id], thread_id)));
    for (;;) {
...
static void _SigHupHandler(SIGNAL_ARGS)
{
    ...
    t_thrd.epochcommit_cxt.got_SIGHUP = true;
...
static void _ShutdownHandler(SIGNAL_ARGS)
{
    ...
    t_thrd.epochcommit_cxt.shutdown_requested = true;
