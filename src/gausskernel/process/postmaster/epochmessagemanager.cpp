...
std::vector<uint64_t> epoch_manager_thread_ids;
...
void EpochMessageManagerMain(void)
{
    sigjmp_buf local_sigjmp_buf;
    MemoryContext epochmessagemanager_context;
     ...
     */
    t_thrd.utils_cxt.CurrentResourceOwner = ResourceOwnerCreate(NULL, "epoch message manager", MEMORY_CONTEXT_STORAGE);
     ...
     */
    epochmessagemanager_context = AllocSetContextCreate(t_thrd.top_mem_cxt,
        "epoch message manager",
        ALLOCSET_DEFAULT_MINSIZE,
        ALLOCSET_DEFAULT_INITSIZE,
        ALLOCSET_DEFAULT_MAXSIZE);
    (void)MemoryContextSwitchTo(epochmessagemanager_context);
    ...
    if (sigsetjmp(local_sigjmp_buf, 1) != 0) {
         ...
         * about in epochmessagemanager, but we do have LWLocks, and perhaps buffers?
         */
        LWLockReleaseAll();
        ...
        MemoryContextResetAndDeleteChildren(epochmessagemanager_context);
     ...
     * sleeping.
     */
    // g_instance.proc_base->epochmessagemanagerLatch = &t_thrd.proc->procLatch;

    pgstat_report_appname("epoch message manager");
    ...
    for(auto &i : epoch_manager_thread_ids ){
        if(i == thread_id){
            break;
        }
        id++;
    }
    if(id < (uint64_t)epoch_manager_thread_ids.size())
        ereport(LOG, (errmsg("epoch manager pid %llu %llu %lu", id, epoch_manager_thread_ids[id], thread_id)));
    for (;;) {
         ...
         */
        if (t_thrd.epochmessagemanager_cxt.got_SIGHUP) {
            t_thrd.epochmessagemanager_cxt.got_SIGHUP = false;
            ProcessConfigFile(PGC_SIGHUP);
        }

        if (t_thrd.epochmessagemanager_cxt.shutdown_requested) {
            /* Normal exit from the epochmessagemanager is here */
            proc_exit(0); /* done */
        }
        FDWEpochMessageManagerThreadMain(id);
...
static void _SigHupHandler(SIGNAL_ARGS)
{
    ...
    t_thrd.epochmessagemanager_cxt.got_SIGHUP = true;
...
static void _ShutdownHandler(SIGNAL_ARGS)
{
    ...
    t_thrd.epochmessagemanager_cxt.shutdown_requested = true;
