...
std::vector<uint64_t> epoch_notify_thread_ids;
...
void EpochNotifyMain(void)
{
    sigjmp_buf local_sigjmp_buf;
    MemoryContext epochnotify_context;
     ...
     */
    t_thrd.utils_cxt.CurrentResourceOwner = ResourceOwnerCreate(NULL, "epoch notify", MEMORY_CONTEXT_STORAGE);
     ...
     */
    epochnotify_context = AllocSetContextCreate(t_thrd.top_mem_cxt,
        "epoch notify",
        ALLOCSET_DEFAULT_MINSIZE,
        ALLOCSET_DEFAULT_INITSIZE,
        ALLOCSET_DEFAULT_MAXSIZE);
    (void)MemoryContextSwitchTo(epochnotify_context);
    ...
    if (sigsetjmp(local_sigjmp_buf, 1) != 0) {
         ...
         * about in epochnotify, but we do have LWLocks, and perhaps buffers?
         */
        LWLockReleaseAll();
        ...
        MemoryContextResetAndDeleteChildren(epochnotify_context);
     ...
     * sleeping.
     */
    // g_instance.proc_base->epochnotifyLatch = &t_thrd.proc->procLatch;

    pgstat_report_appname("epoch notify");
    ...
    for(auto &i : epoch_notify_thread_ids ){
        if(i == thread_id){
            break;
        }
        id++;
    }
    // if(id < (uint64_t)epoch_notify_thread_ids.size())
    //     ereport(LOG, (errmsg("epoch notify pid %llu %llu %lu", id, epoch_notify_thread_ids[id], thread_id)));
    for (;;) {
         ...
         */
        if (t_thrd.epochnotify_cxt.got_SIGHUP) {
            t_thrd.epochnotify_cxt.got_SIGHUP = false;
            ProcessConfigFile(PGC_SIGHUP);
        }

        if (t_thrd.epochnotify_cxt.shutdown_requested) {
            /* Normal exit from the epochnotify is here */
            proc_exit(0); /* done */
        }
        FDWEpochNotifyThreadMain(id);
        pgstat_report_activity(STATE_IDLE, NULL);
    }
}
...
static void _SigHupHandler(SIGNAL_ARGS)
{
    ...
    t_thrd.epochnotify_cxt.got_SIGHUP = true;
...
static void _ShutdownHandler(SIGNAL_ARGS)
{
    ...
    t_thrd.epochnotify_cxt.shutdown_requested = true;
