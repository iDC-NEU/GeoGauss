...
std::vector<uint64_t> epoch_pack_thread_ids;
...
void EpochPackMain(void)
{
    sigjmp_buf local_sigjmp_buf;
    MemoryContext epochpack_context;
    ...
    (void)MemoryContextSwitchTo(epochpack_context);
    ...
    if (sigsetjmp(local_sigjmp_buf, 1) != 0) {
         ...
         * about in epochpack, but we do have LWLocks, and perhaps buffers?
         */
        LWLockReleaseAll();
        ...
        MemoryContextResetAndDeleteChildren(epochpack_context);
    ...
    for(auto &i : epoch_pack_thread_ids ){
        if(i == thread_id){
            break;
        }
        id++;
    ...
    for (;;) {
         ...
         */
        if (t_thrd.epochpack_cxt.got_SIGHUP) {
            t_thrd.epochpack_cxt.got_SIGHUP = false;
            ProcessConfigFile(PGC_SIGHUP);
        }

        if (t_thrd.epochpack_cxt.shutdown_requested) {
            /* Normal exit from the epochpack is here */
            proc_exit(0); /* done */
        }
        FDWEpochPackThreadMain(id, kServerIp, kServerNum, kPackageNum, kNotifyNum, kPackThreadNum, kNotifyThreadNum, local_ip_index);
...
static void _SigHupHandler(SIGNAL_ARGS)
{
    ...
    t_thrd.epochpack_cxt.got_SIGHUP = true;
...
static void _ShutdownHandler(SIGNAL_ARGS)
{
    ...
    t_thrd.epochpack_cxt.shutdown_requested = true;
