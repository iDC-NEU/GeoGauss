...
std::vector<uint64_t> epoch_cache_thread_ids;
...
void EpochMessageCacheManagerMain(void)
{
    sigjmp_buf local_sigjmp_buf;
    MemoryContext epochmessagecachemanager_context;
     ...
     */
    t_thrd.utils_cxt.CurrentResourceOwner = ResourceOwnerCreate(NULL, "epoch message cache manager", MEMORY_CONTEXT_STORAGE);
     ...
     */
    epochmessagecachemanager_context = AllocSetContextCreate(t_thrd.top_mem_cxt,
        "epoch message cache manager",
        ALLOCSET_DEFAULT_MINSIZE,
        ALLOCSET_DEFAULT_INITSIZE,
        ALLOCSET_DEFAULT_MAXSIZE);
    (void)MemoryContextSwitchTo(epochmessagecachemanager_context);
    ...
    if (sigsetjmp(local_sigjmp_buf, 1) != 0) {
         ...
         * about in epochmessagecachemanager, but we do have LWLocks, and perhaps buffers?
         */
        LWLockReleaseAll();
        ...
        MemoryContextResetAndDeleteChildren(epochmessagecachemanager_context);
     ...
     * sleeping.
     */
    // g_instance.proc_base->epochmessagecachemanagerLatch = &t_thrd.proc->procLatch;

    pgstat_report_appname("epoch message cache manager");
    ...
    for(auto &i : epoch_cache_thread_ids ){
        if(i == thread_id){
            break;
        }
        id++;
    ...
    for (;;) {
         ...
         */
        if (t_thrd.epochmessagecachemanager_cxt.got_SIGHUP) {
            t_thrd.epochmessagecachemanager_cxt.got_SIGHUP = false;
            ProcessConfigFile(PGC_SIGHUP);
        }

        if (t_thrd.epochmessagecachemanager_cxt.shutdown_requested) {
            /* Normal exit from the epochmessagecachemanager is here */
            proc_exit(0); /* done */
        }
        FDWEpochMessageCacheManagerThreadMain(id, kServerIp, kServerNum, kPackageNum, kNotifyNum, kPackThreadNum, kNotifyThreadNum, local_ip_index);
...
static void _SigHupHandler(SIGNAL_ARGS)
{
    ...
    t_thrd.epochmessagecachemanager_cxt.got_SIGHUP = true;
...
static void _ShutdownHandler(SIGNAL_ARGS)
{
    ...
    t_thrd.epochmessagecachemanager_cxt.shutdown_requested = true;
