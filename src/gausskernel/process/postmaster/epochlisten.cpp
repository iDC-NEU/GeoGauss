...
void EpochListenMain(void)
{
    sigjmp_buf local_sigjmp_buf;
    MemoryContext epochlisten_context;
    ...
    (void)MemoryContextSwitchTo(epochlisten_context);
    ...
    if (sigsetjmp(local_sigjmp_buf, 1) != 0) {
        ...
        MemoryContextResetAndDeleteChildren(epochlisten_context);
    ...
    for(auto &i : epoch_listen_thread_ids ){
        if(i == thread_id){
            break;
        }
        id++;
    ...
    for (;;) {
        ...
        FDWEpochListenThreadMain(id, port[id]);
...
static void _SigHupHandler(SIGNAL_ARGS)
{
    ...
    t_thrd.epochlisten_cxt.got_SIGHUP = true;
...
static void _ShutdownHandler(SIGNAL_ARGS)
{
    ...
    t_thrd.epochlisten_cxt.shutdown_requested = true;
